...
// Date: 20/03/2025
// Description: it is the gallery.ts file

// it Defines an interface for the image object
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("../data/gallery.json")
        ...
        .then((images: GalleryImage[]) => {
            // it will Get the gallery container element
            const galleryContainer = document.getElementById("gallery-container");

            // it will Check if the gallery container exists
            if (!galleryContainer) {
                console.error("Gallery container not found.");
                return;
            }

            // it will Loop through the images and create HTML elements for each image
            images.forEach((image: GalleryImage) => {
                // Create a column div for the image
                const colDiv = document.createElement("div");
                colDiv.className = "col-md-4 mb-4";

                // it will Create a link element for the image
                ...
                // it is Append the image to the link and the link to the column div
        ...
        .catch((error: Error) => console.error("Error loading gallery images:", error));
});